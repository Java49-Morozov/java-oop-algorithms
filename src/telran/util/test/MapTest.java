...
String[] keys = {"lmn", "abc", "ab", "a", "qwer", "asdf"};
Integer[] values = {3, 7, 2, 1, 16, 4};
protected Map<String, Integer> map;
	...
	void containsKeyTest() {
		...
		assertFalse(map.containsKey("eklmn"));
		assertFalse(map.containsKey("aabbcc"));
	}
	@Test
	...
	void removeTest1() {
		map.remove("lmn");
		String[] Keys1 = {"abc", "ab", "a", "qwer", "asdf"};
		Integer[] Values1 = {7, 2, 1, 16, 4};		
		for(int i = 0; i < Keys1.length; i++) {
			assertEquals(Values1[i], map.get(Keys1[i]));
		}

		map.remove("ab");
		String[] Keys2 = {"abc", "a", "qwer", "asdf"};
		Integer[] Values2 = {7, 1, 16, 4};	
		for(int i = 0; i < Keys2.length; i++) {
			assertEquals(Values2[i], map.get(Keys2[i]));
		}
